...
const updateUI = async () => {
  ...
  try {
    ...
    document.getElementById("temp").innerHTML = `The temperature ${data.city ? `in ${data.city}` : ""} is
        ${Math.round(data.temperature) + "\u2109"}.`;
    if (data.description) {
      document.getElementById("weather").innerHTML = `...and ${data.description}.`;
    }
    if (data.feelings) document.getElementById("content").innerHTML = `You're feeling ${data.feelings}`;
